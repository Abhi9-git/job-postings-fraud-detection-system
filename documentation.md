...
1. **LR scale asymmetry (low risk, deferred).** LR trains on *unscaled* structured values but `app.py` scales at inference. The DT path is exact (trees are scale-invariant) and LR is empirically unaffected — unifying this means retraining LR with the scaler.
2. **`global` token leakage.** The token appears in 100% of fake and 0% of real `CleanData` rows, so main models can hit 1.0 by keying on it. Proper de-leakage must be applied to **both** main models together, never just one (or the 411/345-style drift returns in spirit).
